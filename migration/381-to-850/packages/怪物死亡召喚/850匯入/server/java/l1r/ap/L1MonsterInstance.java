...
import l1r.ao.EliteConfigTable;
import l1r.ao.MonsterDeathSpawnTable;
import l1r.ao.SpawnTable;
...
public class L1MonsterInstance extends L1NpcInstance {
   ...
   private int _eliteTier = 1;
   private final List<EliteConfigTable.MonsterAffix> _affixes = new ArrayList<>();
   private int _physicalReduction = 0;
   private int _magicReduction = 0;
   private int _fireResist = 0;
   private int _waterResist = 0;
   private int _windResist = 0;
   private int _earthResist = 0;
   private long _lastBuffTime = 0;
   private long _lastAttackSpellTime = 0;

   // 怪物死亡召喚 (Monster Death Spawn) 累積強化度與掉落加成
   private int _reincarnationCount = 0;
   private double _reincarnationStrengthPct = 0.0;
   private double _deathSpawnDropBonusPct = 0.0;

   public int getReincarnationCount() {
      return this._reincarnationCount;
   }

   public double getReincarnationStrengthPct() {
      return this._reincarnationStrengthPct;
   }

   public double getDeathSpawnDropBonusPct() {
      return this._deathSpawnDropBonusPct;
   }

   public void setReincarnationData(int count, double strengthPct, double dropBonusPct) {
      this._reincarnationCount = count;
      this._reincarnationStrengthPct = Math.min(100.0, strengthPct);
      this._deathSpawnDropBonusPct = dropBonusPct;
      if (this._reincarnationStrengthPct > 0.0) {
         int bonusHp = (int)(this.ew() * (this._reincarnationStrengthPct / 100.0));
         this.bG(this.ew() + bonusHp);
         this.bx(this.ew());
         this._physicalReduction += (int)(this._reincarnationStrengthPct / 10.0);
      }
   }

   public void handleMonsterDeathSpawn(L1Character killer) {
      try {
         if (Config.MonsterDeathSpawnSwitch != 1) {
            return;
         }
         L1PcInstance player = null;
         if (killer instanceof L1PcInstance) {
            player = (L1PcInstance)killer;
         } else if (killer instanceof L1PetInstance) {
            player = (L1PcInstance)((L1PetInstance)killer).M();
         } else if (killer instanceof L1SummonInstance) {
            player = (L1PcInstance)((L1SummonInstance)killer).M();
         }
         if (player == null || player.eX()) {
            return;
         }

         if (this._reincarnationStrengthPct >= 100.0) {
            return;
         }

         boolean isBoss = this.U_().W();
         MonsterDeathSpawnTable.DeathSpawnRule rule = MonsterDeathSpawnTable.getInstance().findRule(this.z(), isBoss);
         if (rule == null) {
            return;
         }

         int roll = Random.a(1000);
         if (roll >= rule.triggerChanceX10) {
            return;
         }

         double addStrength = rule.strengthPctPerSpawn / 100.0;
         double newStrength = Math.min(rule.maxStrengthPct / 100.0, this._reincarnationStrengthPct + addStrength);
         double newDropBonus = this._deathSpawnDropBonusPct + rule.dropBonusPctPerSpawn;

         int targetX = player.fs();
         int targetY = player.ft();
         int targetMap = player.fp();
         if (rule.teleX > 0 && rule.teleY > 0 && rule.teleMapId >= 0) {
            targetX = rule.teleX;
            targetY = rule.teleY;
            targetMap = rule.teleMapId;
         }

         int spawnNpcId = (rule.spawnNpcId > 0) ? rule.spawnNpcId : this.z();
         L1NpcInstance spawned = SpawnTable.a(spawnNpcId, targetX, targetY, targetMap, Random.a(8), 2, true);
         if (spawned instanceof L1MonsterInstance) {
            L1MonsterInstance newMob = (L1MonsterInstance)spawned;
            newMob.setReincarnationData(this._reincarnationCount + 1, newStrength, newDropBonus);
            newMob.b(new S_SkillSound(newMob.fr(), 7771));
            if (rule.deathTalk != null && !rule.deathTalk.isEmpty()) {
               newMob.b(new S_NpcChatPacket(newMob, rule.deathTalk, 2));
            }
         }
      } catch (Exception e) {
         y.log(Level.WARNING, "handleMonsterDeathSpawn error: " + e.getMessage(), e);
      }
   }
   ...
   public int getEliteTier() {
      return this._eliteTier;
   }

   public List<EliteConfigTable.MonsterAffix> getAffixes() {
      return this._affixes;
   }

   public int getPhysicalReduction() {
      return this._physicalReduction;
   }

   public int getMagicReduction() {
      return this._magicReduction;
   }
   ...
   private void initEliteAndTierSystem(L1Npc npc) {
      try {
         this._eliteTier = EliteConfigTable.getInstance().calculateTier(this.fa());
         if (Config.isEliteMonsterTimeActive()) {
            boolean isBoss = npc.W(); // hard flag
            EliteConfigTable.EliteDifficultyConfig diff = EliteConfigTable.getInstance().getDifficultyConfig(0);
            if (!isBoss && diff != null && diff.mobEliteChancePct > 0) {
               if (Random.a(100) < diff.mobEliteChancePct) {
                  this._isElite = true;
                  // 小怪血量 * 1.3
                  int newHp = (int)(this.ew() * (diff.mobHpMultiplierPct / 100.0));
                  this.bG(newHp);
                  this.bx(newHp);

                  // 隨機賦予前後詞墜
                  if (diff.prefixPool != null && !diff.prefixPool.isEmpty()) {
                     int pId = diff.prefixPool.get(Random.a(diff.prefixPool.size()));
                     EliteConfigTable.MonsterAffix af = EliteConfigTable.getInstance().getAffix(pId);
                     if (af != null) {
                        this._affixes.add(af);
                        applyAffixBonus(af);
                     }
                  }
                  if (diff.suffixPool != null && !diff.suffixPool.isEmpty()) {
                     int sId = diff.suffixPool.get(Random.a(diff.suffixPool.size()));
                     EliteConfigTable.MonsterAffix af = EliteConfigTable.getInstance().getAffix(sId);
                     if (af != null) {
                        this._affixes.add(af);
                        applyAffixBonus(af);
                     }
         ...
         y.log(Level.WARNING, "initEliteAndTierSystem error: " + e.getMessage(), e);
      }
   }

   private void applyAffixBonus(EliteConfigTable.MonsterAffix af) {
      if (af == null) return;
      if (af.addHpPct > 0) {
         int bonus = (int)(this.ew() * (af.addHpPct / 100.0));
         this.bG(this.ew() + bonus);
         this.bx(this.ea() + bonus);
      }
      this._physicalReduction += af.addPhysicalReduction;
      this._magicReduction += af.addMagicReduction;
      this._fireResist += af.addFireResist;
      this._waterResist += af.addWaterResist;
      this._windResist += af.addWindResist;
      this._earthResist += af.addEarthResist;
   }
   ...
   public void b() {
      ...
      // 檢查雙軌施法系統（時段與開關必須啟用）
      if (Config.isEliteMonsterTimeActive() && tryTierSpellCast(var1)) {
         this.v(this.f(this.N(), 2));
         return;
      }
   ...
   private boolean tryTierSpellCast(L1Character target) {
      if (target == null || target.eX() || this.eX()) {
         return false;
      }
      if (this.bB(64)) {
         return false;
      ...
      EliteConfigTable table = EliteConfigTable.getInstance();
      EliteConfigTable.MonsterSpellTierRule rule = table.getTierRule(this._eliteTier);
      if (rule == null) {
         return false;
      }

      // 雙軌之一：治癒 / 增益自保軌
      int hpPct = this.ew() > 0 ? (this.ea() * 100 / this.ew()) : 100;
      if (hpPct <= rule.healHpThresholdPct && now - this._lastBuffTime > 4000L) {
         if (Random.a(100) < rule.buffSpellChancePct) {
            int healSpellId = table.getBestHealSpell(this._eliteTier);
            try {
               L1SkillExecutor exec = LineageUtil.a(healSpellId);
               if (exec != null) {
                  exec.a(this, this.fr(), this.fs(), this.ft(), null);
                  this._lastBuffTime = now;
                  return true;
               }
      ...
      // 增益自保 (如保護罩/鎧甲/加速/聖結界)
      if (now - this._lastBuffTime > 6000L && Random.a(100) < (rule.buffSpellChancePct / 2)) {
         List<Integer> buffs = table.getBuffSpellsUpToTier(this._eliteTier);
         if (buffs != null && !buffs.isEmpty()) {
            int spellId = buffs.get(Random.a(buffs.size()));
            try {
               L1SkillExecutor exec = LineageUtil.a(spellId);
               if (exec != null) {
                  exec.a(this, this.fr(), this.fs(), this.ft(), null);
                  this._lastBuffTime = now;
                  return true;
      ...
      // 雙軌之二：攻擊魔法軌 (看等級區分，最弱怪只會1級魔法，最強怪/BOSS全都會)
      if (now - this._lastAttackSpellTime > 3500L) {
         double dist = this.fu().b(target.fu());
         if (dist <= 10.0) {
            if (Random.a(100) < rule.attackSpellChancePct) {
               List<Integer> attacks = table.getAttackSpellsUpToTier(this._eliteTier);
               if (attacks != null && !attacks.isEmpty()) {
                  int spellId = attacks.get(Random.a(attacks.size()));
                  try {
                     L1SkillExecutor exec = LineageUtil.a(spellId);
                     if (exec != null) {
                        this.ct(this.h(target.fs(), target.ft()));
                        exec.a(this, target.fr(), target.fs(), target.ft(), null);
                        this._lastAttackSpellTime = now;
                        return true;
                     }
                  } catch (Exception ignored) {}
               }
   ...
   public L1MonsterInstance(L1Npc var1) {
      super(var1);
      if (var1 != null) {
         this.initEliteAndTierSystem(var1);
      }
   }
      ...
      if (this.ea() <= 0) {
         ...
         if (this.z() >= 97044 && this.z() <= 97046 && var1.bB(4011)) {
            ...
            var2 = (int)(var2 * 1.5);
                  ...
                  // D系列怪物菁英化減免結算
         if (this._isElite) {
            var2 -= this._physicalReduction;
            if (var1 instanceof L1SummonInstance || var1 instanceof L1PetInstance) {
               EliteConfigTable.EliteDifficultyConfig diff = EliteConfigTable.getInstance().getDifficultyConfig(0);
               int summonReductPct = (diff != null) ? diff.summonAiDmgReductionPct : 30;
               var2 = (int)(var2 * (100 - summonReductPct) / 100.0);
            }
            if (var2 < 1) {
               var2 = 1;
            }
   ...
   public void b(boolean var1) {
      if (this.fe() == 7548 || this.fe() == 7550 || this.fe() == 7552 || this.fe() == 7554 || this.fe() == 7585 || this.fe() == 7591) {
         for (L1PcInstance var6 : L1World.a().f(this)) {
            if (!var6.b((l1r.aq.L1Object)this)) {
               var6.c(this);
            }
         }
         ...
         for (L1PcInstance var5 : L1World.a().f(this)) {
            if (!var5.b((l1r.aq.L1Object)this)) {
               var5.c(this);
            }
         }
         ...
         for (L1PcInstance var4 : L1World.a().f(this)) {
            if (!var4.b((l1r.aq.L1Object)this)) {
               var4.c(this);
            }
         }
         ...
         for (L1PcInstance var2 : L1World.a().f(this)) {
            if (!var2.b((l1r.aq.L1Object)this)) {
               var2.c(this);
            }
         }
   ...
   protected class L1R_a implements Runnable {
      ...
      public void run() {
         try {
            ...
            L1MonsterInstance.this.handleMonsterDeathSpawn(this.b);
