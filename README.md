...
TARGET=L1JTW8.5 / 850
STATE=PLANNING / EVIDENCE-MAPPING
CODE_IMPORT=NONE
DONOR_FILE_COPY=FORBIDDEN
```

這條支線獨立負責 **850 自動狩獵** 的規劃、證據盤點與後續實作；目前階段只更新已確認情報，不先搬入任何其他服務端檔案。

### 已知情報

- 850 是最終目標與架構權威；功能必須依 850 現有核心、設定、DB、封包與客戶端接口落地。
- 後續若進入核心實作，來源優先採 `completed/l1jtw85-core-fixes` 的已修復核心；尚未完成部分才查 `work/l1jtw85-core-fixes`。
- 880 僅作為「自動狩獵核心 / 行為」的參考來源，先盤點類別、入口、狀態機、依賴與必要 invariant，不直接複製檔案。
- 381 僅作為「可參考 AI 模組 / 行為」的來源，先盤點可獨立理解的功能與依賴，不直接複製檔案。
- donor（381 / 880 / 其他版本）的 Java、DB、config、client 檔案目前一律 **不加入此分支**。
- donor 與 850 介面、核心、DB 或封包不同時，一律以 850 為主，只移植必要行為 / invariant，不以 donor 整檔覆蓋 850。

### 目前規劃中的證據圖

```text
1. 880：自動狩獵核心
   → 找入口 / 類別 / 方法 / 狀態 / 執行週期 / 依賴

2. 381：可參考 AI 模組
   → 找可獨立理解的模組 / 行為 / DB-config-client 依賴

3. 850：repaired-core 落點
   → 對照現有核心入口 / config / DB / 封包 / client
   → 決定哪些行為能原生重寫到 850
```
...
- 查資料
- 建 evidence map
- 記錄類別 / 方法 / DB / config / client 依賴
- 寫規劃與驗證條件

現在禁止：
- 複製 381 / 880 / 其他服務端檔案到本分支
- 用 donor 舊核心覆蓋 850 repaired core
- 未確認 850 call path 前直接接入功能
- 為了湊功能先混入 donor DB / config / client 資源
```

此分支的下一步是先完成三張證據圖，再決定 850 原生自動狩獵的最小核心切入點。
