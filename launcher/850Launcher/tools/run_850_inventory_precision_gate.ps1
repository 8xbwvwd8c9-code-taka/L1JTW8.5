...
# Ensure the target did not restart before arming the runtime watch.
...
if (Test-Path -LiteralPath $watchOut) {
    ...
    Write-Host 'STATUS=WATCH_OUTPUT_MISSING'
}
...
Write-Host 'MEM_PRIVATE_SCAN=NO'
Write-Host 'MEMORY_WRITE=NO'
Write-Host 'NEXT=Return the correlation report; if it shows a strong runtime candidate, repeat across a fresh client process before promotion.'
