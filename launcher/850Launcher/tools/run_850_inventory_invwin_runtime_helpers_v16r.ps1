...
$V16Path = Join-Path $OutputDir '850_inventory_invwin_direct_helpers_v16.txt'
$RawPath = Join-Path $OutputDir '850_inventory_invwin_runtime_helpers_v16r_raw.txt'
$OutPath = Join-Path $OutputDir '850_inventory_invwin_runtime_helpers_v16r.txt'
$DecoderDir = Join-Path $PSScriptRoot 'decoder'
$DecoderPath = Join-Path $DecoderDir 'decode_850_inventory_invwin_runtime_helpers_v16r.py'
$SetupPath = Join-Path $DecoderDir 'setup_capstone_decoder.ps1'
$PythonPath = Join-Path $DecoderDir '.venv\Scripts\python.exe'
$CaptureBytes = 384

if (-not (Test-Path -LiteralPath $V16Path)) { throw "Missing V16 target list: $V16Path" }
if (-not (Test-Path -LiteralPath $DecoderPath)) { throw "Missing V16R decoder: $DecoderPath" }
...
foreach ($line in Get-Content -LiteralPath $V16Path -ErrorAction Stop) {
    if ($line -match '^TARGET_RVA=0x([0-9A-Fa-f]+)\s') {
        [void]$targets.Add([Convert]::ToInt64($Matches[1],16))
    }
}
if ($targets.Count -lt 1) { throw 'V16R target list empty.' }
if (-not $targets.Contains(0x004CC180L) -or -not $targets.Contains(0x004CE990L)) {
    throw 'V16R blocked: known runtime anchor helpers are missing from V16 target list.'
}
...
public static class InvWinV16RMem850 {
    ...
    public static InvWinV16RRegion850 Query(int pid, long address) {
        IntPtr h = Open(pid); try {
            ...
            return new InvWinV16RRegion850{Base=m.BaseAddress.ToInt64(),Size=unchecked((long)m.RegionSize.ToUInt64()),State=m.State,Protect=m.Protect,Type=m.Type};
        } finally { CloseHandle(h); }
    }
    public static bool IsExecutableImage(InvWinV16RRegion850 r) {
...
$lines.Add('MODE=850_INVENTORY_INVWIN_RUNTIME_HELPER_CAPTURE_V16R')
...
foreach ($rva in ($targets | Sort-Object)) {
    $va = $base + [long]$rva
    if ($va -lt $base -or $va -ge $limit) { throw ('Target RVA outside main module: 0x{0:X8}' -f $rva) }
    $region = [InvWinV16RMem850]::Query($proc.Id,$va)
    if (-not [InvWinV16RMem850]::IsExecutableImage($region)) { throw ('Target not executable MEM_IMAGE: RVA=0x{0:X8}' -f $rva) }
    $available = ([long]$region.Base + [long]$region.Size) - $va
    $want = [int][Math]::Min([long]$CaptureBytes,$available)
    $bytes = [InvWinV16RMem850]::ReadBytes($proc.Id,$va,$want)
...
if ($LASTEXITCODE -ne 0) { throw "V16R decoder failed: exit=$LASTEXITCODE" }
if (-not (Test-Path -LiteralPath $OutPath)) { throw "V16R output missing: $OutPath" }

Write-Host 'STATUS=PASS_INVWIN_RUNTIME_HELPERS_V16R_EXECUTED'
