...
$RawPath = Join-Path $OutputDir '850_inventory_invwin_16c_element_helpers_v17_raw.txt'
$OutPath = Join-Path $OutputDir '850_inventory_invwin_16c_element_helpers_v17.txt'
$DecoderDir = Join-Path $PSScriptRoot 'decoder'
$DecoderPath = Join-Path $DecoderDir 'decode_850_inventory_invwin_16c_element_helpers_v17.py'
$SetupPath = Join-Path $DecoderDir 'setup_capstone_decoder.ps1'
$PythonPath = Join-Path $DecoderDir '.venv\Scripts\python.exe'
$CaptureBytes = 384

# Two proven runtime sanity anchors plus the exact +0x16C iterator lane.
$TargetRvas = @(
    0x004CC180L, # INDEX_ACCESSOR sanity anchor
    0x004CE990L, # SIZE_ACCESSOR sanity anchor
    0x00869B30L, # INVWIN+0x16C traversal / lookup function
    0x0084EFF0L, # iterator/init helper
    0x00880E80L, # begin-like adapter used by 0x00869B30
    0x008816E0L, # end-like adapter used by 0x00869B30
    0x00854E30L, # iterator advance helper
    0x008547A0L, # iterator compare helper
    0x00854AA0L, # iterator dereference wrapper
    0x00854A50L  # exact wrapper landing needed to classify dereference semantics
)

if (-not (Test-Path -LiteralPath $DecoderPath)) { throw "Missing V17 decoder: $DecoderPath" }
...
public static class InvWinV17Mem850 {
    ...
    public static InvWinV17Region850 Query(int pid, long address) {
        IntPtr h = Open(pid); try {
            ...
            return new InvWinV17Region850{Base=m.BaseAddress.ToInt64(),Size=unchecked((long)m.RegionSize.ToUInt64()),State=m.State,Protect=m.Protect,Type=m.Type};
        } finally { CloseHandle(h); }
    }
    public static bool IsExecutableImage(InvWinV17Region850 r) {
...
$lines.Add('MODE=850_INVENTORY_INVWIN_16C_ELEMENT_HELPERS_V17')
...
$lines.Add('FOCUS=INVWIN_PLUS_0x16C_ELEMENT_IDENTITY')
...
foreach ($rva in $TargetRvas) {
    ...
    $region = [InvWinV17Mem850]::Query($proc.Id,$va)
    if (-not [InvWinV17Mem850]::IsExecutableImage($region)) { throw ('Target not executable MEM_IMAGE: RVA=0x{0:X8}' -f $rva) }
    $available = ([long]$region.Base + [long]$region.Size) - $va
    $want = [int][Math]::Min([long]$CaptureBytes,$available)
    $bytes = [InvWinV17Mem850]::ReadBytes($proc.Id,$va,$want)
...
if ($LASTEXITCODE -ne 0) { throw "V17 decoder failed: exit=$LASTEXITCODE" }
if (-not (Test-Path -LiteralPath $OutPath)) { throw "V17 output missing: $OutPath" }

Write-Host 'STATUS=PASS_INVWIN_16C_ELEMENT_HELPERS_V17_EXECUTED'
