...
foreach ($line in $decoderLines) {
    ...
    $decoderHits[$n] = [pscustomobject]@{
        N = $n
        Tid = [int]$m.Groups[2].Value
        Value = $m.Groups[3].Value.ToUpperInvariant()
        EipRva = $m.Groups[5].Value.ToUpperInvariant()
...
foreach ($n in ($watchHits.Keys | Sort-Object)) {
    ...
    $decoderPass =
        $null -ne $d -and
        $d.Status -eq 'PASS_UNIQUE_TARGET_WRITER' -and
        $d.EipRva -eq $w.EipRva -and
        $d.Tid -eq $w.Tid
    $eipRva = Hex-ToUInt64 $w.EipRva
    $matched = @()
    if ($decoderPass -and $null -ne $eipRva) {
...
$decoderPassCount = @($records | Where-Object { $_.DecoderPass }).Count
...
$out.Add("DECODER_UNIQUE_WRITERS=$decoderPassCount")
...
$out.Add('[CORRELATION]')
foreach ($r in $records) {
    $decoderStatus = if ($null -ne $r.Decoder) { $r.Decoder.Status } else { 'MISSING' }
    if ($r.Stores.Count -eq 0) {
        $out.Add("HIT N=$($r.N) VALUE=$($r.Watch.Value) EIP_RVA=$($r.Watch.EipRva) DECODER=$decoderStatus MATCH=NONE")
    }
    else {
        foreach ($s in $r.Stores) {
            $out.Add(('HIT N={0} VALUE={1} EIP_RVA={2} DECODER={3} MATCH={4} LITERAL_RVA=0x{5:X8} IMM32={6}' -f $r.N, $r.Watch.Value, $r.Watch.EipRva, $decoderStatus, $s.Kind, $s.LiteralRva, $s.Imm32))
        }
    }
}
$out.Add('')
