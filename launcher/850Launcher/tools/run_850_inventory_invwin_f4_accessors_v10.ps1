...
$RawPath = Join-Path $OutputDir '850_inventory_invwin_f4_accessors_v10_raw.txt'
$OutPath = Join-Path $OutputDir '850_inventory_invwin_f4_accessors_v10.txt'
$DecoderDir = Join-Path $PSScriptRoot 'decoder'
$Setup = Join-Path $DecoderDir 'setup_capstone_decoder.ps1'
$Decoder = Join-Path $DecoderDir 'decode_850_inventory_invwin_f4_accessors_v10.py'
$VenvPython = Join-Path $DecoderDir '.venv\Scripts\python.exe'

$Accessors = [ordered]@{
    BEGIN_MUT   = 0x004CD870L
    BEGIN_CONST = 0x004CD890L
    END_MUT     = 0x004CD8B0L
    END_CONST   = 0x004CD8D0L
}
$ByteLimit = 96
...
if($gate['CLIENT_SHA256'] -ne $ExpectedSha256 -or $gate['STATUS'] -ne 'PASS_ROOT_OWNER_RESTART_STABLE' -or $gate['OWNER_ANCHOR'] -ne 'PASS_RESTART_STABLE'){
    throw 'V10 blocked: restart-stable ROOT owner gate has not passed.'
}
...
public static class V10ImageRead850 {
    ...
    public static byte[] ReadExecImage(int pid,long address,int count){
        ...
        try {
            ...
            int n=(int)got.ToInt64(); if(n==b.Length) return b; byte[] t=new byte[n]; Array.Copy(b,t,n); return t;
...
$lines.Add('MODE=850_INVENTORY_INVWIN_F4_ACCESSOR_CAPTURE_V10')
...
$lines.Add("ACCESSOR_BYTE_LIMIT=$ByteLimit")
...
$lines.Add('')
$lines.Add('[ACCESSORS]')
foreach($kv in $Accessors.GetEnumerator()){
    $rva=[long]$kv.Value; $va=$base+$rva
    if($va -lt $base -or $va -ge $limit){ throw "Accessor outside module: $($kv.Key)" }
    $bytes=[V10ImageRead850]::ReadExecImage($proc.Id,$va,$ByteLimit)
    $hex=($bytes | ForEach-Object { $_.ToString('X2') }) -join ' '
    $lines.Add(("ACCESSOR NAME={0} RVA=0x{1:X8} VA=0x{2:X8} BYTES={3}" -f $kv.Key,$rva,$va,$hex))
}
...
if($LASTEXITCODE -ne 0){ throw "V10 decoder failed: exit=$LASTEXITCODE" }
if(-not (Test-Path -LiteralPath $OutPath)){ throw "V10 output missing: $OutPath" }
Write-Host 'STATUS=PASS_V10_F4_ACCESSOR_CAPTURE_AND_DECODE'
