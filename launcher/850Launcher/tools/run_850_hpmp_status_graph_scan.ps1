...
$moduleBytes=[HpmpGraphMem850]::Read($pid,$base,$size)
...
function ReadObj([long]$a,[int]$n=0x320){$b=[HpmpGraphMem850]::Read($pid,$a,$n);if($b.Length-lt4){return $null};$vt=[BitConverter]::ToUInt32($b,0);$vc=VtCount $vt 16;return [pscustomobject]@{A=$a;B=$b;VT=$vt;VC=$vc;ASCII=(Ascii $b 16)}}
...
$lines.Add("TIME=$((Get-Date).ToString('yyyy-MM-dd HH:mm:ss'))");$lines.Add('MODE=850_HPMP_STATUS_GRAPH_SCAN');$lines.Add("PID=$pid");$lines.Add("CLIENT=$([IO.Path]::GetFullPath($ClientPath))");$lines.Add("CLIENT_SHA256=$sha");$lines.Add('CLIENT_AUTHORITY=1');$lines.Add('MEMORY_WRITE=NO');$lines.Add('SOURCE_MODIFIED=NO');$lines.Add('')
...
Write-Host 'STATUS=PASS';Write-Host "PID=$pid";Write-Host "DIRECT_CHILD_OBJECTS=$direct";Write-Host "SEMANTIC_CHILD_OBJECTS=$semantic";Write-Host "OUTPUT=$OutputPath";Write-Host 'MEMORY_WRITE=NO'
