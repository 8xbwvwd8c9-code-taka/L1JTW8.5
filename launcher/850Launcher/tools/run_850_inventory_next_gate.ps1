...
foreach ($tool in $tools) {
    ...
    try {
        & $tool.Path -ClientPath $full
        $results.Add([pscustomobject]@{ Name = $tool.Name; Status = 'PASS_EXECUTED'; Error = '' })
        Write-Host "END=$($tool.Name) STATUS=PASS_EXECUTED"
    } catch {
        $msg = $_.Exception.Message
        $results.Add([pscustomobject]@{ Name = $tool.Name; Status = 'FAIL'; Error = $msg })
