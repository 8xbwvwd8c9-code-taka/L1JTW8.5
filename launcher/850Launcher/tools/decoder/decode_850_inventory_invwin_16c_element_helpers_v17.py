...
def main():
    ap = argparse.ArgumentParser(description="Decode exact runtime helpers for INVWIN+0x16C element identity V17")
    ...
    if sha != EXPECTED_SHA or authority != "1" or exact != "YES" or heap != "NO" or priv != "NO" or write != "NO":
        raise SystemExit("V17 authority/safety gate failed")
    if focus != "INVWIN_PLUS_0x16C_ELEMENT_IDENTITY":
        raise SystemExit(f"V17 focus mismatch: {focus}")
    got = {rva for rva, _ in targets}
    if got != EXPECTED_TARGETS:
        missing = sorted(EXPECTED_TARGETS - got)
        extra = sorted(got - EXPECTED_TARGETS)
        raise SystemExit(f"V17 exact-target mismatch missing={missing} extra={extra}")
    ...
    out = []
    out.append("MODE=850_INVENTORY_INVWIN_16C_ELEMENT_HELPER_DECODE_V17")
    ...
    out.append("FOCUS=INVWIN_PLUS_0x16C_ELEMENT_IDENTITY")
    out.append("HELPER_DEPTH=1")
    ...
    out.append("STATUS=" + ("PASS_16C_ELEMENT_HELPERS_CAPTURE_VALIDATED" if anchor_ok else "BLOCKED_RUNTIME_HELPER_ANCHOR_MISMATCH"))
    out.append("AUTO_PROMOTION_ALLOWED=NO")
    out.append("ELEMENT_IDENTITY_PROVEN=NO")
    out.append("ITEM_RECORD_TRAVERSAL_PROVEN=NO")
    out.append("OBJECT_ID_SEMANTICS_PROVEN=NO")
    out.append("ITEM_ID_SEMANTICS_PROVEN=NO")
    out.append("COUNT_SEMANTICS_PROVEN=NO")
    ...
    dst.write_text("\n".join(out) + "\n", encoding="utf-8")
    if not anchor_ok:
        raise SystemExit("V17 known runtime helper anchor validation failed")
