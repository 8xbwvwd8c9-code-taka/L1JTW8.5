...
$RawPath = Join-Path $OutputDir '850_inventory_invwin_f4_helpers_v9_raw.txt'
$OutPath = Join-Path $OutputDir '850_inventory_invwin_f4_helpers_v9.txt'
$DecoderDir = Join-Path $PSScriptRoot 'decoder'
$Setup = Join-Path $DecoderDir 'setup_capstone_decoder.ps1'
$Decoder = Join-Path $DecoderDir 'decode_850_inventory_invwin_f4_helpers_v9.py'
$VenvPython = Join-Path $DecoderDir '.venv\Scripts\python.exe'

# Runtime helper RVAs proven by INVWIN callers that use ECX=this+0xF4.
$Helpers = [ordered]@{
    SIZE_CANDIDATE  = 0x004CE990L
    INDEX_CANDIDATE = 0x004CC180L
    HELPER_80E20    = 0x00880E20L
    HELPER_81680    = 0x00881680L
    HELPER_828F0    = 0x008828F0L
    HELPER_82800    = 0x00882800L
    HELPER_54820    = 0x00854820L
    HELPER_550E0    = 0x008550E0L
}
$ByteLimit = 384
...
if ($gate['CLIENT_SHA256'] -ne $ExpectedSha256 -or $gate['STATUS'] -ne 'PASS_ROOT_OWNER_RESTART_STABLE' -or $gate['OWNER_ANCHOR'] -ne 'PASS_RESTART_STABLE') {
    throw 'V9 blocked: restart-stable ROOT owner gate has not passed.'
}
...
public static class V9ImageRead850 {
...
$lines.Add('MODE=850_INVENTORY_INVWIN_F4_HELPER_CAPTURE_V9')
...
$lines.Add("HELPER_BYTE_LIMIT=$ByteLimit")
...
foreach ($kv in $Helpers.GetEnumerator()) {
    ...
    if ($va -lt $base -or $va -ge $limit) { throw "Helper outside module: $($kv.Key)" }
    $bytes=[V9ImageRead850]::ReadExecImage($proc.Id,$va,$ByteLimit)
    $hex=($bytes | ForEach-Object { $_.ToString('X2') }) -join ' '
    $lines.Add(("HELPER NAME={0} RVA=0x{1:X8} VA=0x{2:X8} BYTES={3}" -f $kv.Key,$rva,$va,$hex))
}
...
if ($LASTEXITCODE -ne 0) { throw "V9 decoder failed: exit=$LASTEXITCODE" }
if (-not (Test-Path -LiteralPath $OutPath)) { throw "V9 output missing: $OutPath" }

Write-Host 'STATUS=PASS_V9_F4_HELPER_CAPTURE_AND_DECODE'
