...
BUILD=Path("recovery/protobuf-runtime-source-cfr-build")
LOG=Path("recovery/protobuf_runtime_source_cfr_javac.log")
STATE=Path("recovery/protobuf_runtime_source_cfr_compile.json")
LIST=Path("recovery/protobuf_runtime_source_cfr_files.txt")
...
state={"gate":"PROTOBUF_RUNTIME_CFR_SOURCE_ONLY_EXPERIMENT","source_root":str(SRC),"binary_runtime_on_classpath":False,
"java_sources_submitted":len(sources),"compile_exit_code":proc.returncode,"generated_class_files_total":len(classes),
"expected_runtime_class_files":len(expected),"missing_runtime_classes":len(missing),"extra_runtime_classes":len(extra),
"class_set_pass":proc.returncode==0 and not missing and not extra,"javac_error_headers":len(headers),"error_file_count":len(files),
"top_error_files":files.most_common(50),"top_error_messages":msgs.most_common(50)}
