...
TMP=Path('recovery/compile-ref-protobuf-l1rpb.builder-current-c-x.jar')
OUT=Path('recovery/protobuf_builder_current_c_x_alias_experiment.json')
MD=Path('recovery/PROTOBUF_BUILDER_CURRENT_C_X_ALIAS_EXPERIMENT.md')

TARGET='l1rpb/a$a.class'
SRC=('a','(Ll1rpb/x;)Ll1rpb/a$a;')
ALIAS=('c','(Ll1rpb/x;)Ll1rpb/a$a;')
ERASED=('c','(Ll1rpb/x;)Ll1rpb/x$a;')
...
if name_idx is None: raise SystemExit('missing Utf8 c')
...
state={
 'experiment':'BUILDER_CURRENT_C_X_TYPED_ALIAS_A_A_POST176',
 'target_class':TARGET,
 ...
 'source_collision_gate':'run229 normalization removed 44 builder c(x) VF bridges; residual c(x)=0',
 ...
 'pass_signal':'44 current c(x) obligations disappear or advance; parser/nonprotobuf remain 0'
}
OUT.write_text(json.dumps(state,indent=2)+'\n',encoding='utf-8')
MD.write_text(
 '# Builder Current c(x) Alias A/B — Post-176\n\n'
 '- Adds exactly one recovery compile-ref typed alias at a$a.\n'
 '- Run229 removed all 44 conflicting concrete builder c(x) source bridges; residual=0.\n'
 '- Donor provider-map proves synthetic c(x)->x$a delegates to typed a(x)->a$a with TBuilderType Signature.\n'
