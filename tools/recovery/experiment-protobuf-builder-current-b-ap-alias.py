...
TMP=Path('recovery/compile-ref-protobuf-l1rpb.builder-current-b-ap.jar')
OUT=Path('recovery/protobuf_builder_current_b_ap_alias_experiment.json')
MD=Path('recovery/PROTOBUF_BUILDER_CURRENT_B_AP_ALIAS_EXPERIMENT.md')

TARGET='l1rpb/p$a.class'
SRC=('d','(Ll1rpb/ap;)Ll1rpb/p$a;')
ALIAS=('b','(Ll1rpb/ap;)Ll1rpb/p$a;')
ERASED=('b','(Ll1rpb/ap;)Ll1rpb/x$a;')
...
state={
 'experiment':'BUILDER_CURRENT_B_AP_TYPED_ALIAS_P_A',
 'target_class':TARGET,
 ...
 'source_collision_scan':'zero b(ap) declarations found across PBMessageALL*.java recovered source',
 'provider_choice':'nearest raw superclass p$a only; do not expose duplicate a$a alias in this A/B',
 ...
 'pass_signal':'44 current b(ap) obligations disappear or advance; parser/nonprotobuf remain 0'
}
OUT.write_text(json.dumps(state,indent=2)+'\n',encoding='utf-8')
MD.write_text(
 '# Builder Current b(ap) Alias A/B\n\n'
 '- Adds exactly one recovery compile-ref typed alias at p$a.\n'
 '- Source collision scan: zero exact b(ap) declarations.\n'
 '- Uses nearest p$a donor bridge -> typed d(ap)->p$a / TBuilderType.\n'
 '- Does not expose duplicate a$a b(ap) alias.\n'
