...
for p in sorted((STAGE / "l1r" / "an").glob("*.java")):
    ...
    # Fully qualified runtime references emitted by Vineflower, e.g. a.h, a.k.g.
    # A standalone token a. is rewritten only when it is not preceded by an
    # identifier, dot, or dollar sign.
    text, n2 = re.subn(r"(?<![A-Za-z0-9_.$])a\.", "l1rpb.", text)
