...
TMP=Path('recovery/compile-ref-protobuf-l1rpb.builder-current-j.jar')
OUT=Path('recovery/protobuf_builder_current_j_alias_experiment.json')
MD=Path('recovery/PROTOBUF_BUILDER_CURRENT_J_ALIAS_EXPERIMENT.md')
...
if name_idx is None: raise SystemExit('missing Utf8 j')
...
state={
 'experiment':'BUILDER_CURRENT_J_TYPED_ALIAS_P_A_POST176',
 'target_class':TARGET,
 ...
 'source_collision_gate':'run229 normalization removed 44 builder j() VF bridges; residual j()=0',
 'provider_choice':'nearest raw superclass p$a only; do not expose duplicate a$a j() alias',
 ...
 'pass_signal':'44 current j() obligations disappear or advance; parser/nonprotobuf remain 0'
}
OUT.write_text(json.dumps(state,indent=2)+'\n',encoding='utf-8')
MD.write_text(
 '# Builder Current j() Alias A/B — Post-176\n\n'
 '- Adds exactly one recovery compile-ref typed alias at p$a.\n'
 '- Run229 removed all 44 conflicting concrete builder j() source bridges; residual=0.\n'
 '- Donor provider-map proves p$a synthetic j()->x$a delegates to typed ah()->p$a with TBuilderType Signature.\n'
 '- Duplicate a$a j() alias is intentionally not exposed in this A/B.\n'
