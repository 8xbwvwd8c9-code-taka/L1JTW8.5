...
TMP=Path('recovery/compile-ref-protobuf-l1rpb.builder-current-e-ap.jar')
OUT=Path('recovery/protobuf_builder_current_e_ap_alias_experiment.json')
MD=Path('recovery/PROTOBUF_BUILDER_CURRENT_E_AP_ALIAS_EXPERIMENT.md')

TARGET='l1rpb/p$a.class'
SRC=('c','(Ll1rpb/ap;)Ll1rpb/p$a;')
ALIAS=('e','(Ll1rpb/ap;)Ll1rpb/p$a;')
ERASED=('e','(Ll1rpb/ap;)Ll1rpb/x$a;')
...
if name_idx is None: raise SystemExit('missing Utf8 e')
...
state={
 'experiment':'BUILDER_CURRENT_E_AP_TYPED_ALIAS_P_A',
 'target_class':TARGET,
 ...
 'source_collision_scan':'zero e(ap) declarations found across PBMessageALL*.java recovered source',
 'provider_choice':'single donor-proven p$a provider; no duplicate lower-level alias exists in provider map',
 ...
 'pass_signal':'44 current e(ap) obligations disappear or advance; parser/nonprotobuf remain 0'
}
OUT.write_text(json.dumps(state,indent=2)+'\n',encoding='utf-8')
MD.write_text(
 '# Builder Current e(ap) Alias A/B\n\n'
 '- Adds exactly one recovery compile-ref typed alias at p$a.\n'
 '- Source collision scan: zero exact e(ap) declarations.\n'
 '- Donor provider-map proves synthetic e(ap)->x$a delegates to typed c(ap)->p$a with TBuilderType Signature.\n'
