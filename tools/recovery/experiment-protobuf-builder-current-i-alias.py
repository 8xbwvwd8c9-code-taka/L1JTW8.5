...
TMP=Path('recovery/compile-ref-protobuf-l1rpb.builder-current-i.jar')
OUT=Path('recovery/protobuf_builder_current_i_alias_experiment.json')
MD=Path('recovery/PROTOBUF_BUILDER_CURRENT_I_ALIAS_EXPERIMENT.md')
...
if name_idx is None: raise SystemExit('missing Utf8 i')
...
state={
 'experiment':'BUILDER_CURRENT_I_TYPED_ALIAS_P_A_POST176',
 'target_class':TARGET,
 ...
 'source_collision_gate':'run229 normalization removed 44 builder i() VF bridges; residual i()=0',
 'provider_choice':'p$a only; a$a i() provider rejected because target d() is abstract',
 ...
 'pass_signal':'44 current i() obligations disappear or advance; parser/nonprotobuf remain 0'
}
OUT.write_text(json.dumps(state,indent=2)+'\n',encoding='utf-8')
MD.write_text(
 '# Builder Current i() Alias A/B — Post-176\n\n'
 '- Adds exactly one recovery compile-ref typed alias at p$a.\n'
 '- Run229 removed all 44 conflicting concrete builder i() source bridges; residual=0.\n'
 '- Donor provider-map proves p$a synthetic i()->x$a delegates to typed ai()->p$a with TBuilderType Signature.\n'
 '- a$a i() path is intentionally not exposed because its target d() is abstract.\n'
