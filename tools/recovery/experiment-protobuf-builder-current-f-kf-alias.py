...
TMP=Path('recovery/compile-ref-protobuf-l1rpb.builder-current-f-kf.jar')
OUT=Path('recovery/protobuf_builder_current_f_kf_alias_experiment.json')
MD=Path('recovery/PROTOBUF_BUILDER_CURRENT_F_KF_ALIAS_EXPERIMENT.md')

TARGET='l1rpb/p$a.class'
SRC=('d','(Ll1rpb/k$f;)Ll1rpb/p$a;')
ALIAS=('f','(Ll1rpb/k$f;)Ll1rpb/p$a;')
ERASED=('f','(Ll1rpb/k$f;)Ll1rpb/x$a;')
...
if name_idx is None: raise SystemExit('missing Utf8 f')
...
state={
 'experiment':'BUILDER_CURRENT_F_KF_TYPED_ALIAS_P_A',
 'target_class':TARGET,
 ...
 'source_collision_scan':'zero f(k$f) declarations found across PBMessageALL*.java recovered source',
 ...
 'pass_signal':'44 current f(k$f) obligations disappear or advance; parser/nonprotobuf remain 0'
}
OUT.write_text(json.dumps(state,indent=2)+'\n',encoding='utf-8')
MD.write_text(
 '# Builder Current f(k$f) Alias A/B\n\n'
 '- Adds exactly one recovery compile-ref typed alias at p$a.\n'
 '- Source collision scan: zero exact f(k$f) declarations.\n'
 '- Donor provider-map proves synthetic f(k$f)->x$a delegates to typed d(k$f)->p$a with TBuilderType Signature.\n'
