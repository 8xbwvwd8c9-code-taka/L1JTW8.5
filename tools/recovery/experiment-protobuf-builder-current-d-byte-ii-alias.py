...
TMP=Path('recovery/compile-ref-protobuf-l1rpb.builder-current-d-byte-ii.jar')
OUT=Path('recovery/protobuf_builder_current_d_byte_ii_alias_experiment.json')
MD=Path('recovery/PROTOBUF_BUILDER_CURRENT_D_BYTE_II_ALIAS_EXPERIMENT.md')

TARGET='l1rpb/a$a.class'
SRC=('a','([BII)Ll1rpb/a$a;')
ALIAS=('d','([BII)Ll1rpb/a$a;')
ERASED=('d','([BII)Ll1rpb/x$a;')
...
state={
 'experiment':'BUILDER_CURRENT_D_BYTE_II_TYPED_ALIAS',
 'target_class':TARGET,
 ...
 'source_collision_scan':'zero d(byte[],int,int) declarations found across PBMessageALL*.java recovered source',
 ...
 'pass_signal':'44 current d(byte[],int,int) obligations disappear or advance; parser/nonprotobuf remain 0'
}
OUT.write_text(json.dumps(state,indent=2)+'\n',encoding='utf-8')
MD.write_text(
 '# Builder Current d(byte[],int,int) Alias A/B\n\n'
 '- Adds exactly one recovery compile-ref typed alias.\n'
 '- Source collision scan: zero exact d(byte[],int,int) declarations.\n'
