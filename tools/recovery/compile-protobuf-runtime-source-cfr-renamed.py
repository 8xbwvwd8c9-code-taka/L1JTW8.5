...
SRC=Path("recovery/protobuf-runtime-src-cfr-renamed")
BUILD=Path("recovery/protobuf-runtime-source-cfr-renamed-build")
LOG=Path("recovery/protobuf_runtime_source_cfr_renamed_javac.log")
STATE=Path("recovery/protobuf_runtime_source_cfr_renamed_compile.json")
LIST=Path("recovery/protobuf_runtime_source_cfr_renamed_files.txt")
...
headers=[x for x in lines if ": error: " in x]; files=Counter(); msgs=Counter()
for line in headers:
    try:
...
state={"gate":"PROTOBUF_RUNTIME_CFR_RENAMED_SOURCE_ONLY_EXPERIMENT","source_root":str(SRC),
