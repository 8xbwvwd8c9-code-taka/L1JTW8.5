...
OUT=Path("recovery/protobuf-runtime-src-cfr-renamed")
STATE=Path("recovery/protobuf_runtime_source_cfr_renamed_decompile.json")
if len(sys.argv)!=2: raise SystemExit("usage: decompile-protobuf-runtime-cfr-renamed.py <cfr.jar>")
CFR=Path(sys.argv[1])
if OUT.exists(): shutil.rmtree(OUT)
OUT.mkdir(parents=True)
cmd=["java","-jar",str(CFR),str(SRC),"--outputdir",str(OUT),"--silent","true",
     "--renameillegalidents","true","--renamedupmembers","true"]
...
state={"source_jar":str(SRC),"cfr_exit_code":p.returncode,"runtime_class_files":len(classes),
       "java_files_copied":len(java),"renameillegalidents":True,"renamedupmembers":True,
       "package":"l1rpb","gameplay_logic_changed":False}
STATE.write_text(json.dumps(state,indent=2)+"\n",encoding="utf-8")
print(json.dumps(state,indent=2))
if p.returncode!=0 or not java: raise SystemExit("CFR renamed runtime decompile failed")
