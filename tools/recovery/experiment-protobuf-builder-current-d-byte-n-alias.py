...
TMP=Path('recovery/compile-ref-protobuf-l1rpb.builder-current-d-byte-n.jar')
OUT=Path('recovery/protobuf_builder_current_d_byte_n_alias_experiment.json')
MD=Path('recovery/PROTOBUF_BUILDER_CURRENT_D_BYTE_N_ALIAS_EXPERIMENT.md')

TARGET='l1rpb/a$a.class'
SRC=('a','([BLl1rpb/n;)Ll1rpb/a$a;')
ALIAS=('d','([BLl1rpb/n;)Ll1rpb/a$a;')
ERASED=('d','([BLl1rpb/n;)Ll1rpb/x$a;')
...
state={
 'experiment':'BUILDER_CURRENT_D_BYTE_N_TYPED_ALIAS',
 'target_class':TARGET,
 ...
 'source_collision_scan':'zero d(byte[],n) declarations found across PBMessageALL*.java normalized source',
 ...
 'pass_signal':'44 current d(byte[],n) obligations disappear or advance; parser/nonprotobuf remain 0'
}
OUT.write_text(json.dumps(state,indent=2)+'\n',encoding='utf-8')
MD.write_text(
 '# Builder Current d(byte[],n) Alias A/B\n\n'
 '- Adds exactly one recovery compile-ref typed alias.\n'
 '- Source collision scan across normalized PBMessageALL*.java: zero exact d(byte[],n) declarations.\n'
